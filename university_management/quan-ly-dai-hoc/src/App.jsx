...
import { User, BookUser, Banknote, LayoutDashboard, Plus, X, Edit, Trash2, Search, AlertCircle, LogOut, FileUp, Users, ShieldCheck, CreditCard, Landmark, Home, UserCheck, Library, Briefcase, Calendar, Star, Users2, ChevronDown, Moon, Sun, Sparkles, Send, Bot } from 'lucide-react';
...
function AppContent() {
  ...
  const handleLogin = (username, password) => {
    ...
    const allStudents = JSON.parse(localStorage.getItem('students')) || initialStudents; 
    ...
    if (user) {
        ...
        if (user.role === 'lecturer' && user.lecturerId) {
            ...
            const studentInfo = allStudents.find(s => s.id === user.studentId);
            if (studentInfo) {
                userData.name = studentInfo.name;
            }
  ...
  const roleComponent = {
      admin: <AdminApplication currentUser={currentUser} onLogout={handleLogout} onOpenAiModal={() => setIsAiModalOpen(true)} />,
      student: <StudentPortalApplication currentUser={currentUser} onLogout={handleLogout} onOpenAiModal={() => setIsAiModalOpen(true)} />,
      lecturer: <LecturerApplication currentUser={currentUser} onLogout={handleLogout} onOpenAiModal={() => setIsAiModalOpen(true)} />,
      head_of_department: <LecturerApplication currentUser={currentUser} onLogout={handleLogout} onOpenAiModal={() => setIsAiModalOpen(true)} />,
  };
  
  return (
    <>
        {roleComponent[currentUser.role] || <LoginScreen onLogin={handleLogin}/>}
        {isAiModalOpen && <AIAssistantModal onClose={() => setIsAiModalOpen(false)} currentUser={currentUser} />}
    </>
  );
}
  ...
  return ( 
    <div className="flex items-center justify-center h-screen bg-gray-100 dark:bg-gray-900">
      <div className="w-full max-w-md p-8 space-y-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg">
        <h2 className="text-3xl font-bold text-center text-gray-800 dark:text-gray-100">Cổng Thông Tin Đại Học</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="text-sm font-bold text-gray-600 dark:text-gray-300 block">Tên đăng nhập</label>
            <input 
              type="text" 
              value={username} 
              onChange={(e) => setUsername(e.target.value)} 
              className="w-full p-3 mt-2 text-gray-700 dark:text-gray-200 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-300 dark:border-gray-600" 
              required 
          ...
          <div>
            <button type="submit" className="w-full py-3 mt-4 font-bold text-white bg-red-600 rounded-lg hover:bg-red-700">Đăng nhập</button>
          </div>
          <div className="text-center text-sm text-gray-500 dark:text-gray-400 pt-4 border-t dark:border-gray-600">
            ...
            <p><span className="font-medium">Giảng viên (Trưởng BM):</span> gv_a / password</p>
            <p><span className="font-medium">Sinh viên:</span> sinhvien_an / password</p>
          </div>
        </form>
      </div>
    </div> 
...
// === Admin Application ===
const AdminApplication = ({ currentUser, onLogout, onOpenAiModal }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900 font-sans">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto"><HeaderAdmin currentUser={currentUser} onLogout={onLogout} onOpenAiModal={onOpenAiModal} /><div className="space-y-6">{renderView()}</div></main>
...
const StudentPortalApplication = ({ currentUser, onLogout, onOpenAiModal }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900 font-sans">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto">
        <HeaderStudent currentUser={currentUser} studentName={myStudent?.name} onLogout={onLogout} onOpenAiModal={onOpenAiModal} />
        {renderView()}
      </main>
...
const LecturerApplication = ({ currentUser, onLogout, onOpenAiModal }) => {
    const [currentView, setCurrentView] = useState('grades');
    const allStudents = JSON.parse(localStorage.getItem('students')) || [];
    ...
    const myDepartmentMembers = useMemo(() => { 
        if (currentUser.role !== 'head_of_department') return []; 
        return allLecturers.filter(t => t.facultyId === currentUser.facultyId); 
    }, [allLecturers, currentUser]);
    ...
    return (
        <div className="flex h-screen bg-gray-100 dark:bg-gray-900 font-sans">
            ...
            <main className="flex-1 p-4 md:p-8 overflow-y-auto">
                <HeaderAdmin currentUser={currentUser} onLogout={onLogout} onOpenAiModal={onOpenAiModal} />
                {renderView()}
            </main>
        </div>
    );
};
const AIAssistantModal = ({ onClose, currentUser }) => {
    const API_KEY = "YOUR_API_KEY_HERE";

    const [messages, setMessages] = useState([{
        sender: 'ai',
        text: `Xin chào ${currentUser.name || 'bạn'}, tôi là Trợ lý AI học vụ. Bạn cần tôi giúp gì?`
    }]);
    ...
    const handleSendMessage = async (e) => {
        ...
        if (!API_KEY || API_KEY === "YOUR_API_KEY_HERE") {
            setMessages(prev => [...prev, {
                sender: 'ai',
                text: 'Lỗi: Khóa API của Gemini chưa được cung cấp. Vui lòng cập nhật trong mã nguồn.'
            }]);
            setIsLoading(false);
            return;
        }
        ...
        const allData = {};
        const dataKeys = ['students', 'lecturers', 'courses', 'classSections', 'schedules', 'enrollments', 'grades', 'fees', 'faculties'];
        dataKeys.forEach(key => {
            allData[key] = JSON.parse(localStorage.getItem(key) || '[]');
        });
        
        const systemPrompt = `Bạn là một trợ lý AI học vụ thông minh và hữu ích trong một ứng dụng quản lý đại học. Nhiệm vụ của bạn là trả lời các câu hỏi của người dùng dựa trên dữ liệu JSON được cung cấp. Hãy trả lời một cách ngắn gọn, chính xác và thân thiện.
        Thông tin người dùng hiện tại: ${JSON.stringify(currentUser)}
        Dữ liệu hệ thống: <span class="math-inline">\{JSON\.stringify\(allData\)\}
Câu hỏi của người dùng\: "</span>{input}"`;
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error?.message || 'Có lỗi xảy ra khi gọi API Gemini.');
            }
            ...
            setMessages(prev => [...prev, { sender: 'ai', text: `Rất tiếc, đã có lỗi xảy ra: ${error.message}` }]);
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl h-[80vh] flex flex-col">
                <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700">
                    <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2"><Bot className="text-red-500" /> Trợ lý AI Học vụ</h3>
                    <button onClick={onClose} className="p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full"><X size={24} /></button>
                </div>
                <div className="flex-1 p-4 overflow-y-auto space-y-4">
                    {messages.map((msg, index) => (
                        <div key={index} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
                            {msg.sender === 'ai' && <div className="w-8 h-8 rounded-full bg-red-500 flex items-center justify-center text-white shrink-0"><Bot size={20}/></div>}
                            <div className={`max-w-[75%] p-3 rounded-2xl ${msg.sender === 'user' ? 'bg-red-500 text-white rounded-br-none' : 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded-bl-none'}`}><p className="text-sm" style={{ whiteSpace: 'pre-wrap' }}>{msg.text}</p></div>
                        </div>
                    ))}
                    {isLoading && (
                        <div className="flex items-end gap-2">
                            <div className="w-8 h-8 rounded-full bg-red-500 flex items-center justify-center text-white shrink-0"><Bot size={20}/></div>
                             <div className="max-w-[75%] p-3 rounded-2xl bg-gray-200 dark:bg-gray-700"><div className="flex items-center gap-1"><span className="h-2 w-2 bg-red-500 rounded-full animate-bounce [animation-delay:-0.3s]"></span><span className="h-2 w-2 bg-red-500 rounded-full animate-bounce [animation-delay:-0.15s]"></span><span className="h-2 w-2 bg-red-500 rounded-full animate-bounce"></span></div></div>
                        </div>
                    )}
                ...
                <div className="p-4 border-t border-gray-200 dark:border-gray-700">
                    <form onSubmit={handleSendMessage} className="flex items-center gap-2">
                        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Nhập câu hỏi của bạn ở đây..." className="flex-1 p-3 border border-gray-300 dark:border-gray-600 rounded-full bg-gray-50 dark:bg-gray-700 text-gray-800 dark:text-gray-200" />
                        <button type="submit" disabled={isLoading || !input.trim()} className="p-3 bg-red-500 text-white rounded-full hover:bg-red-600 disabled:bg-gray-400"><Send size={20} /></button>
                    </form>
                </div>
            </div>
        </div>
...
const LecturerManagementView = ({ allLecturers, faculties, onAdd, onEdit, onDelete }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const filteredLecturers = useMemo(() => allLecturers.filter(t => t.name.toLowerCase().includes(searchTerm.toLowerCase())), [allLecturers, searchTerm]);

    const lecturerRows = filteredLecturers.map(t => {
        const faculty = faculties.find(d => d.id === t.facultyId);
        return (
            <tr key={t.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td className="p-4">{lecturerRoles[t.role] || t.role}</td>
                <td className="p-4">{faculty?.name || 'Chưa có'}</td>
                <td className="p-4 text-center space-x-2">
                    <button onClick={() => onEdit(t)} className="p-2 text-red-600"><Edit size={18} /></button>
                    <button onClick={() => onDelete(t.id)} className="p-2 text-red-600"><Trash2 size={18} /></button>
                </td>
            </tr>
        );
    });

    return (<div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm"><div className="flex justify-between items-center mb-4 flex-wrap gap-4"><h3 className="text-xl font-bold text-gray-700 dark:text-gray-200">Quản lý Giảng viên</h3><div className="flex items-center gap-2 flex-grow sm:flex-grow-0"><SearchBar searchTerm={searchTerm} setSearchTerm={setSearchTerm} placeholder="Tìm giảng viên..." /><button onClick={onAdd} className="bg-red-500 text-white px-4 py-2 rounded-lg">Thêm</button></div></div><div className="overflow-x-auto"><table className="w-full text-left text-gray-800 dark:text-gray-300"><thead className="text-gray-700 dark:text-gray-200"><tr className="bg-gray-50 dark:bg-gray-700"><th className="p-4 font-semibold">Họ tên</th><th className="p-4 font-semibold">Giới tính</th><th className="p-4 font-semibold">Chức vụ</th><th className="p-4 font-semibold">Khoa</th><th className="p-4 font-semibold text-center">Hành động</th></tr></thead><tbody>{lecturerRows}</tbody></table></div></div>);
...
const CourseManagementView = ({ courses, faculties, onAdd, onEdit, onDelete }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const filteredCourses = useMemo(() => courses.filter(c => c.name.toLowerCase().includes(searchTerm.toLowerCase()) || c.code.toLowerCase().includes(searchTerm.toLowerCase())), [courses, searchTerm]);
    ...
    const courseRows = filteredCourses.map(c => {
        ...
        return (
            <tr key={c.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td className="p-4 text-center">{c.credits}</td>
                <td className="p-4">{faculty?.name || '---'}</td>
                <td className="p-4 text-center space-x-2">
                    <button onClick={() => onEdit(c)} className="p-2 text-red-600"><Edit size={18} /></button>
    ...
    return (<div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm"><div className="flex justify-between items-center mb-4 flex-wrap gap-4"><h3 className="text-xl font-bold text-gray-700 dark:text-gray-200">Quản lý Học phần</h3><div className="flex items-center gap-2 flex-grow sm:flex-grow-0"><SearchBar searchTerm={searchTerm} setSearchTerm={setSearchTerm} placeholder="Tìm học phần..." /><button onClick={onAdd} className="bg-red-500 text-white px-4 py-2 rounded-lg">Thêm</button></div></div><div className="overflow-x-auto"><table className="w-full text-left text-gray-800 dark:text-gray-300"><thead className="text-gray-700 dark:text-gray-200"><tr className="bg-gray-50 dark:bg-gray-700"><th className="p-4 font-semibold">Mã học phần</th><th className="p-4 font-semibold">Tên học phần</th><th className="p-4 font-semibold text-center">Số tín chỉ</th><th className="p-4 font-semibold">Khoa</th><th className="p-4 font-semibold text-center">Hành động</th></tr></thead><tbody>{courseRows}</tbody></table></div></div>);
...
const ClassSectionManagementView = ({ classSections, lecturers, enrollments, courses, onAdd, onEdit, onDelete }) => {
    const classRows = classSections.map(c => {
        const lecturer = lecturers.find(t => t.id === c.lecturerId);
        const course = courses.find(cr => cr.id === c.courseId);
        const enrolledStudentsCount = enrollments.filter(e => e.classSectionId === c.id).length;
        return (
            <tr key={c.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                <td className="p-4">{c.name}</td>
                <td className="p-4">{course ? `${course.name} (${course.code})` : '---'}</td>
                <td className="p-4">{lecturer ? `${lecturer.title} ${lecturer.name}` : 'Chưa phân công'}</td>
                <td className="p-4">{enrolledStudentsCount}</td>
                <td className="p-4 text-center space-x-2">
                    <button onClick={() => onEdit(c)} className="p-2 text-red-600"><Edit size={18} /></button>
                    <button onClick={() => onDelete(c.id)} className="p-2 text-red-600"><Trash2 size={18} /></button>
                </td>
...
const StudentProfileView = ({ allStudents, onAdd, onEdit, onDelete }) => {
    ...
    const studentRows = filteredStudents.map(s => (
        <tr key={s.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
            ...
            <td className="p-4">{s.studentCode}</td>
            <td className="p-4">{s.dob}</td>
            <td className="p-4">{s.major}</td>
            <td className="p-4 text-center space-x-2">
                <button onClick={() => onEdit(s)} className="p-2 text-red-600"><Edit size={18} /></button>
                <button onClick={() => onDelete(s.id)} className="p-2 text-red-600"><Trash2 size={18} /></button>
            </td>
        </tr>
    ));

    return ( <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm"> <div className="flex justify-between items-center mb-4 flex-wrap gap-4"> <h3 className="text-xl font-bold text-gray-700 dark:text-gray-200">Hồ sơ Sinh viên</h3><div className="flex items-center gap-2 flex-grow sm:flex-grow-0"> <SearchBar searchTerm={searchTerm} setSearchTerm={setSearchTerm} placeholder="Tìm theo tên hoặc MSSV..." /> <button onClick={() => onAdd()} className="bg-red-500 text-white px-4 py-2 rounded-lg shrink-0">Thêm SV</button> </div> </div> <div className="overflow-x-auto"><table className="w-full text-left text-gray-800 dark:text-gray-300"><thead className="text-gray-700 dark:text-gray-200"><tr className="bg-gray-50 dark:bg-gray-700"><th className="p-4 font-semibold">Họ tên</th><th className="p-4 font-semibold">MSSV</th><th className="p-4 font-semibold">Ngày sinh</th><th className="p-4 font-semibold">Ngành học</th><th className="p-4 font-semibold text-center">Hành động</th></tr></thead><tbody>{studentRows}</tbody></table></div> </div> );
...
const GradeManagementView = ({ students, grades, setGrades, classSections, courses }) => {
    ...
    const allComponentKeysForCourse = useMemo(() => {
        if (!selectedCourseId) return [];
        const keys = new Set();
        // Scan all existing grades for this course to find every component used
        grades.forEach(g => {
            if (g.courseId === selectedCourseId && g.components) {
                Object.keys(g.components).forEach(key => keys.add(key));
            }
        });
        // If no grades with components exist yet, provide a default set.
        if (keys.size === 0) {
            keys.add('Chuyên cần');
            keys.add('Giữa kỳ');
        }
        ...
        setGrades(prevGrades => {
            ...
            if (gradeRecordIndex > -1) {
                ...
                const newGrade = { 
                    ...
                    components: {}, 
                    final: null,
                    weights: {} // Default weights, might need a better system
                };
                if (type === 'final') {
    ...
    const calculateOverallGrade = (grade) => {
        ...
        // Fallback: If no weights are defined, calculate a simple average
        if (totalWeight === 0) {
        ...
        if (isEditing) {
            return (
                <input
                    ...
                    autoFocus
                    className="w-16 text-center bg-yellow-100 dark:bg-yellow-800 border-red-500 border-2 rounded"
                />
            );
        }
                                ...
                                <tr key={student.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    ...
                                    <td className="p-2 border border-gray-300 dark:border-gray-600 text-sm">
                                        <div className="flex flex-wrap justify-center items-center gap-x-4 gap-y-2">
                                            {allComponentKeysForCourse.map(key => (
                                                <span key={key} className="inline-block p-1 whitespace-nowrap">
                                                    {key}: <EditableCell studentId={student.id} courseId={selectedCourseId} type="component" componentKey={key} value={components[key]} />
                                                </span>
                                            ))}
                                        </div>
                                    </td>
...
const FeeManagementView = ({ allStudents, fees, setFees }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const filteredStudents = useMemo(() => allStudents.filter(student => student.name.toLowerCase().includes(searchTerm.toLowerCase()) || student.studentCode.includes(searchTerm)), [allStudents, searchTerm]);
    const [selectedStudentId, setSelectedStudentId] = useState(filteredStudents.length > 0 ? filteredStudents[0].id : null);
    useEffect(() => { if (!filteredStudents.find(s => s.id === selectedStudentId)) { setSelectedStudentId(filteredStudents.length > 0 ? filteredStudents[0].id : null); } }, [filteredStudents, selectedStudentId]);
    const [newFee, setNewFee] = useState({ feeName: '', amount: '', dueDate: '' });
    const studentFees = useMemo(() => fees.filter(f => f.studentId === selectedStudentId), [fees, selectedStudentId]);
    const handleFeeChange = (e) => setNewFee({ ...newFee, [e.target.name]: e.target.value });
    const handleAddFeeSubmit = (e) => { e.preventDefault(); const amount = parseFloat(newFee.amount); if (!newFee.feeName || isNaN(amount) || !newFee.dueDate) return alert("Vui lòng nhập đầy đủ thông tin."); const newId = fees.length > 0 ? Math.max(...fees.map(i => i.id)) + 1 : 1; setFees([...fees, { id: newId, studentId: selectedStudentId, feeName: newFee.feeName, amount, dueDate: newFee.dueDate, status: 'Chưa thanh toán' }]); setNewFee({ feeName: '', amount: '', dueDate: '' }); };
    const toggleStatus = (feeId) => setFees(fees.map(f => f.id === feeId ? { ...f, status: f.status === 'Đã thanh toán' ? 'Chưa thanh toán' : 'Đã thanh toán' } : f));
    ...
    const feeRows = studentFees.map(f => (
        <tr key={f.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
            <td className="p-4">{f.feeName}</td>
            <td className="p-4">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(f.amount)}</td>
            <td className="p-4">{f.dueDate}</td>
            <td className="p-4 text-center"><span className={`px-2 py-1 text-xs rounded-full ${f.status === 'Đã thanh toán' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300'}`}>{f.status}</span></td>
            <td className="p-4 text-center"><button onClick={() => toggleStatus(f.id)} className="text-sm bg-indigo-500 text-white px-3 py-1 rounded">Đổi</button></td>
        </tr>
    ));

    return (<div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm space-y-6"><div className="flex items-center gap-4 flex-wrap"><label className="font-medium shrink-0 text-gray-800 dark:text-gray-200">Quản lý học phí cho:</label><div className="flex-grow min-w-[200px]"><SearchBar searchTerm={searchTerm} setSearchTerm={setSearchTerm} placeholder="Tìm sinh viên..."/></div><div className="flex-grow min-w-[200px]"><FormSelect name="studentId" value={selectedStudentId || ''} onChange={(e) => setSelectedStudentId(Number(e.target.value))}>{filteredStudents.length > 0 ? filteredStudents.map(s => <option key={s.id} value={s.id}>{s.name} ({s.studentCode})</option>) : <option>Không tìm thấy sinh viên</option>}</FormSelect></div></div>{selectedStudentId ? <div className="overflow-x-auto"><table className="w-full text-left text-gray-800 dark:text-gray-300"><thead className="text-gray-700 dark:text-gray-200"><tr className="bg-gray-50 dark:bg-gray-700"><th className="p-4 font-semibold">Khoản thu</th><th className="p-4 font-semibold">Số tiền</th><th className="p-4 font-semibold">Hạn nộp</th><th className="p-4 font-semibold text-center">Trạng thái</th><th className="p-4 font-semibold text-center">Hành động</th></tr></thead><tbody>{studentFees.length > 0 ? feeRows : <tr><td colSpan="5" className="text-center p-8 text-gray-500 dark:text-gray-400">Không có khoản phí nào.</td></tr>}</tbody><tfoot className="text-gray-800 dark:text-gray-200"><tr className="bg-gray-50 dark:bg-gray-700"><td className="p-2"><input type="text" name="feeName" value={newFee.feeName} onChange={handleFeeChange} placeholder="Tên khoản thu" className="w-full border rounded p-2 bg-white dark:bg-gray-600 border-gray-300 dark:border-gray-500"/></td><td className="p-2"><input type="number" name="amount" value={newFee.amount} onChange={handleFeeChange} placeholder="Số tiền" className="w-full border rounded p-2 bg-white dark:bg-gray-600 border-gray-300 dark:border-gray-500"/></td><td className="p-2"><input type="date" name="dueDate" value={newFee.dueDate} onChange={handleFeeChange} className="w-full border rounded p-2 bg-white dark:bg-gray-600 border-gray-300 dark:border-gray-500"/></td><td className="p-2 text-center" colSpan="2"><button onClick={handleAddFeeSubmit} className="bg-red-500 text-white px-4 py-2 rounded-lg w-full">Thêm</button></td></tr></tfoot></table></div> : <p className="text-gray-500 dark:text-gray-400">Vui lòng chọn một sinh viên.</p>}</div>);
};const UserManagementView = ({ users, onAdd, onEdit, onDelete }) => {
    // SỬA LỖI: Tạo mảng các hàng (rows) trước khi render
    const userRows = users.map(user => (
        <tr key={user.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
            ...
            <td className="p-4"><span className={`px-2 py-1 text-xs rounded-full ${user.role === 'admin' ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300' : user.role === 'lecturer' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300'}`}>{user.role}</span></td>
            <td className="p-4 text-center space-x-2">
                <button onClick={() => onEdit(user)} className="p-2 text-red-600 disabled:opacity-50" disabled={user.role === 'admin'}><Edit size={18} /></button>
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-gray-700 dark:text-gray-200">Danh sách Tài khoản</h3>
                <button onClick={onAdd} className="bg-red-500 text-white px-4 py-2 rounded-lg">Thêm Tài khoản</button>
            </div>
            <div className="overflow-x-auto">
...
const SidebarStudent = ({ currentView, setCurrentView }) => { const navItems = [ { id: 'grades', label: 'Kết quả học tập', icon: BookUser }, { id: 'fees', label: 'Thông tin Học phí', icon: Banknote }, ]; return ( <nav className="w-20 lg:w-64 bg-white dark:bg-gray-800 shadow-lg"><div className="p-4 lg:p-6 border-b border-gray-200 dark:border-gray-700"><h1 className="text-xl lg:text-2xl font-bold text-red-600 text-center lg:text-left"><span className="lg:hidden">SV</span><span className="hidden lg:inline">Cổng Sinh viên</span></h1></div><ul className="mt-6">{navItems.map(item => (<li key={item.id} className="px-2 lg:px-4 mb-2"><button onClick={() => setCurrentView(item.id)} className={`w-full flex items-center justify-center lg:justify-start p-3 rounded-lg ${currentView === item.id ? 'bg-red-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-300 hover:bg-red-50 dark:hover:bg-gray-700'}`}><item.icon className="h-6 w-6 lg:mr-4" /> <span className="hidden lg:inline font-medium">{item.label}</span></button></li>))}</ul></nav> ); };
const HeaderStudent = ({ currentUser, studentName, onLogout, onOpenAiModal }) => ( <header className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm mb-6 flex justify-between items-center flex-wrap gap-2"><div><h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Sinh viên: {studentName}</h2><p className="text-gray-500 dark:text-gray-400">Chào mừng, {currentUser.name.replace('SV: ','')}</p></div><div className="flex items-center gap-3"><ThemeToggleButton /><button onClick={onOpenAiModal} title="Trợ lý AI" className="p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full"><Sparkles className="text-red-500" size={22} /></button><button onClick={onLogout} title="Đăng xuất" className="p-2 text-gray-500 dark:text-gray-400 hover:bg-red-100 hover:text-red-600 dark:hover:bg-red-900/50 rounded-full"><LogOut size={22} /></button></div></header>);
const StudentGradeView = ({ grades, courses }) => {
    const getGradeForLetter = (score) => { if (score >= 9.0) return 'A+'; if (score >= 8.5) return 'A'; if (score >= 8.0) return 'B+'; if (score >= 7.0) return 'B'; if (score >= 6.5) return 'C+'; if (score >= 5.5) return 'C'; if (score >= 5.0) return 'D+'; if (score >= 4.0) return 'D'; return 'F'; };
    
    const gradeRows = grades.map(grade => {
        const course = courses.find(c => c.id === grade.courseId);
        if (!course) return null;
        const overallScore = grade.final; // Simplified for this view
        return (
            <tr key={grade.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                <td className="p-4 font-medium">{course.code}</td>
                <td className="p-4">{course.name}</td>
                <td className="p-4 text-center">{grade.components ? Object.values(grade.components).join('; ') : '-'}</td>
                <td className="p-4 text-center">{grade.final?.toFixed(1) || '-'}</td>
                <td className="p-4 font-bold text-red-600 dark:text-red-400 text-center">{overallScore != null ? getGradeForLetter(overallScore) : '-'}</td>
            </tr>
        );
    });
    
    return ( <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm"><h3 className="text-xl font-bold mb-4 text-gray-700 dark:text-gray-200">Bảng điểm học kỳ</h3><div className="overflow-x-auto"><table className="w-full text-left text-gray-800 dark:text-gray-300"><thead className="text-gray-700 dark:text-gray-200"><tr className="bg-gray-50 dark:bg-gray-700"><th className="p-4 font-semibold">Mã HP</th><th className="p-4 font-semibold">Tên học phần</th><th className="p-4 font-semibold text-center">Điểm QT</th><th className="p-4 font-semibold text-center">Điểm CK</th><th className="p-4 font-semibold text-center">Điểm chữ</th></tr></thead><tbody>{gradeRows}</tbody></table></div></div> );
};const StudentFeeView = ({ fees, onPay }) => {
    const feeRows = fees.map(f => (
        <tr key={f.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
            ...
            <td className="p-4">{f.dueDate}</td>
            <td className="p-4 text-center"><span className={`px-2 py-1 text-xs rounded-full ${f.status === 'Đã thanh toán' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300'}`}>{f.status}</span></td>
            <td className="p-4 text-center">{f.status === 'Chưa thanh toán' ? <button onClick={() => onPay(f)} className="py-1 px-4 bg-green-500 text-white text-sm rounded-md hover:bg-green-600">Thanh toán</button> : <ShieldCheck className="w-5 h-5 text-green-500 mx-auto" />}</td>
        </tr>
    ));
    return ( <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm"><h3 className="text-xl font-bold mb-4 text-gray-700 dark:text-gray-200">Các khoản phí</h3><div className="overflow-x-auto"><table className="w-full text-left text-gray-800 dark:text-gray-300"><thead className="text-gray-700 dark:text-gray-200"><tr className="bg-gray-50 dark:bg-gray-700"><th className="p-4 font-semibold">Tên khoản thu</th><th className="p-4 font-semibold">Số tiền</th><th className="p-4 font-semibold">Hạn nộp</th><th className="p-4 font-semibold text-center">Trạng thái</th><th className="p-4 font-semibold text-center">Hành động</th></tr></thead><tbody>{fees.length > 0 ? feeRows : <tr><td colSpan="5" className="text-center p-8 text-gray-500 dark:text-gray-400">Không có khoản phí nào.</td></tr>}</tbody></table></div></div> );
};const PaymentModal = ({ fee, onClose, onConfirm }) => { const [paymentMethod, setPaymentMethod] = useState('bank'); return ( <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50"><div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-8 w-full max-w-md m-4"><div className="flex justify-between items-center mb-4"><h3 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Thanh toán Học phí</h3><button onClick={onClose} className="p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full"><X size={24} /></button></div><div className="space-y-4 text-gray-800 dark:text-gray-200"><p><span className="font-semibold">Khoản phí:</span> {fee.feeName}</p><p className="text-2xl font-bold text-red-600">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(fee.amount)}</p><div className="pt-4 border-t border-gray-200 dark:border-gray-600"><p className="font-semibold mb-2">Chọn phương thức:</p><div className="space-y-2"><label className="flex items-center p-3 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700"><input type="radio" name="paymentMethod" value="bank" checked={paymentMethod === 'bank'} onChange={(e) => setPaymentMethod(e.target.value)} className="h-4 w-4 text-red-600 bg-gray-100 border-gray-300 focus:ring-red-500 dark:focus:ring-red-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" /><span className="ml-3 flex items-center gap-2"><Landmark size={20}/> Chuyển khoản</span></label><label className="flex items-center p-3 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700"><input type="radio" name="paymentMethod" value="momo" checked={paymentMethod === 'momo'} onChange={(e) => setPaymentMethod(e.target.value)} className="h-4 w-4 text-red-600 bg-gray-100 border-gray-300 focus:ring-red-500 dark:focus:ring-red-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" /><span className="ml-3 flex items-center gap-2"><img src="https://upload.wikimedia.org/wikipedia/vi/f/fe/MoMo_Logo.png" alt="MoMo" className="w-5 h-5"/> Ví MoMo</span></label><label className="flex items-center p-3 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700"><input type="radio" name="paymentMethod" value="card" checked={paymentMethod === 'card'} onChange={(e) => setPaymentMethod(e.target.value)} className="h-4 w-4 text-red-600 bg-gray-100 border-gray-300 focus:ring-red-500 dark:focus:ring-red-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" /><span className="ml-3 flex items-center gap-2"><CreditCard size={20}/> Thẻ tín dụng</span></label></div></div><div className="flex justify-end gap-4 pt-4"><button type="button" onClick={onClose} className="py-2 px-4 bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-100 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500">Hủy</button><button type="button" onClick={onConfirm} className="py-2 px-6 bg-green-500 text-white rounded-lg hover:bg-green-600 shadow">Xác nhận</button></div></div></div></div> ); };
const SidebarLecturer = ({ currentUser, currentView, setCurrentView }) => { const navItems = [ { id: 'grades', label: 'Quản lý điểm', icon: BookUser }, { id: 'schedule', label: 'TKB Cá nhân', icon: Calendar }, ]; if(currentUser.role === 'head_of_department') { navItems.push({ id: 'department', label: 'Bộ môn', icon: Users2 }); } return ( <nav className="w-20 lg:w-64 bg-white dark:bg-gray-800 shadow-lg"><div className="p-4 lg:p-6 border-b border-gray-200 dark:border-gray-700"><h1 className="text-xl lg:text-2xl font-bold text-red-600 text-center lg:text-left"><span className="lg:hidden">GV</span><span className="hidden lg:inline">Cổng Giảng viên</span></h1></div><ul className="mt-6">{navItems.map(item => (<li key={item.id} className="px-2 lg:px-4 mb-2"><button onClick={() => setCurrentView(item.id)} className={`w-full flex items-center justify-center lg:justify-start p-3 rounded-lg ${currentView === item.id ? 'bg-red-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-300 hover:bg-red-50 dark:hover:bg-gray-700'}`}><item.icon className="h-6 w-6 lg:mr-4" /> <span className="hidden lg:inline font-medium">{item.label}</span></button></li>))}</ul></nav> ); };
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
            ...
            <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                    <thead className="text-gray-700 dark:text-gray-200">
                        <tr className="bg-gray-100 dark:bg-gray-700">
                            <th className="p-2 border border-gray-300 dark:border-gray-600">Tiết</th>
                            {days.map(day => <th key={day} className="p-2 border border-gray-300 dark:border-gray-600">{day}</th>)}
                        </tr>
                    </thead>
                    <tbody className="text-gray-800 dark:text-gray-300">
                        {periods.map(period => (
                            <tr key={period}>
                                <td className="p-2 border border-gray-300 dark:border-gray-600 font-bold bg-gray-50 dark:bg-gray-700/50 text-center">{period}</td>
                                {days.map(day => {
                                    const s = schedule.find(item => item.day === day && item.period === period);
                                    return (
                                        <td key={day} className="p-2 border border-gray-300 dark:border-gray-600 text-center h-16 align-middle">
                                            {s ? (
                                                <>
                                                    <span className="font-semibold block">{s.courseName}</span>
                                                    <span className="text-sm text-gray-500 block">{s.className}</span>
                                                </>
                                            ) : null}
                                        </td>
                                    );
...
const ScheduleManagementView = ({ schedules, setSchedules, lecturers, classSections, courses }) => {
    const [editing, setEditing] = useState(null);
    const [form, setForm] = useState({ lecturerId: '', classSectionId: '', days: [], periods: [] });
    const dayOptions = ['Thứ 2', 'Thứ 3', 'Thứ 4', 'Thứ 5', 'Thứ 6', 'Thứ 7'];
    const periodOptions = Array.from({ length: 12 }, (_, i) => i + 1);
    const handleEdit = (schedule) => { setEditing(schedule); setForm({ lecturerId: schedule.lecturerId, classSectionId: schedule.classSectionId, days: [schedule.day], periods: [schedule.period] }); };
    const handleDelete = (id) => { if (window.confirm("Bạn có chắc chắn muốn xóa lịch học này?")) { setSchedules(schedules.filter(a => a.id !== id)); } };
    const handleChange = (e) => { const { name, value } = e.target; const isNumericField = ['lecturerId', 'classSectionId'].includes(name); const processedValue = isNumericField && value ? parseInt(value, 10) : value; setForm({ ...form, [name]: processedValue }); };
    const handleMultiSelectChange = (name, values) => { setForm({ ...form, [name]: values }); };
    const handleSubmit = (e) => { e.preventDefault(); const { lecturerId, classSectionId, days, periods } = form; if (!lecturerId || !classSectionId || days.length === 0 || periods.length === 0) { alert("Vui lòng điền đầy đủ thông tin."); return; } const checkConflict = (day, period, currentScheduleId = null) => { return schedules.find(a => a.day === day && a.period === period && (a.lecturerId === lecturerId || a.classSectionId === classSectionId) && a.id !== currentScheduleId); }; if (editing) { const day = days[0]; const period = periods[0]; const conflict = checkConflict(day, period, editing.id); if (conflict) { alert(`Lịch bị trùng!`); return; } setSchedules(schedules.map(a => a.id === editing.id ? { ...a, lecturerId, classSectionId, day, period } : a )); } else { const newSchedules = []; let isConflict = false; let lastId = schedules.length > 0 ? Math.max(...schedules.map(a => a.id)) : 0; for (const day of days) { for (const period of periods) { const conflict = checkConflict(day, period); if (conflict) { alert(`Lịch bị trùng!`); isConflict = true; break; } newSchedules.push({ id: ++lastId, lecturerId, classSectionId, day, period }); } if(isConflict) break; } if (!isConflict) { setSchedules([...schedules, ...newSchedules]); } } handleCancel(); };
    const handleCancel = () => { setEditing(null); setForm({ lecturerId: '', classSectionId: '', days: [], periods: [] }); };

    const scheduleRows = schedules.sort((a,b) => dayOptions.indexOf(a.day) - dayOptions.indexOf(b.day) || a.period - b.period).map(a => {
        const lecturer = lecturers.find(t => t.id === a.lecturerId);
        const cls = classSections.find(c => c.id === a.classSectionId);
        const course = courses.find(s => s.id === cls?.courseId);
        return (
            <tr key={a.id} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                <td className="p-4">{lecturer ? `${lecturer.title} ${lecturer.name}` : '---'}</td>
                <td className="p-4">{cls?.name || '---'}</td>
                <td className="p-4">{course?.name || '---'}</td>
                <td className="p-4">{a.day}</td>
                <td className="p-4 text-center">{a.period}</td>
                <td className="p-4 text-center space-x-2">
                    <button onClick={() => handleEdit(a)} className="p-2 text-red-600"><Edit size={18} /></button>
                    <button onClick={() => handleDelete(a.id)} className="p-2 text-red-600"><Trash2 size={18} /></button>
                </td>
            </tr>
        );
    });

    return ( <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm space-y-6"> <h3 className="text-xl font-bold text-gray-700 dark:text-gray-200">Xếp Lịch học</h3> <div className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800/50 space-y-4"> <h4 className="font-semibold text-lg text-gray-800 dark:text-gray-200">{editing ? "Chỉnh sửa Lịch học" : "Thêm Lịch học mới"}</h4> <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-start"> <FormSelect label="Giảng viên" name="lecturerId" value={form.lecturerId || ''} onChange={handleChange} required> <option value="">-- Chọn GV --</option> {lecturers.map(t => <option key={t.id} value={t.id}>{`${t.title} ${t.name}`}</option>)} </FormSelect> <FormSelect label="Lớp tín chỉ" name="classSectionId" value={form.classSectionId || ''} onChange={handleChange} required> <option value="">-- Chọn lớp --</option> {classSections.map(c => <option key={c.id} value={c.id}>{c.name}</option>)} </FormSelect> <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 items-end"> <MultiSelectDropdown label="Ngày" options={dayOptions} selectedValues={form.days} onChange={(values) => handleMultiSelectChange('days', values)} /> <MultiSelectDropdown label="Tiết" options={periodOptions} selectedValues={form.periods} onChange={(values) => handleMultiSelectChange('periods', values.sort((a,b) => a-b))} /> <div className="flex gap-2 col-span-full md:col-span-1 self-end"> <button type="submit" className="w-full bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600"> {editing ? "Cập nhật" : "Thêm"} </button> {editing && ( <button type="button" onClick={handleCancel} className="w-full bg-gray-300 dark:bg-gray-600 text-black dark:text-white px-4 py-2 rounded-lg hover:bg-gray-400 dark:hover:bg-gray-500"> Hủy </button> )} </div> </div> </form> </div> <div className="overflow-x-auto"> <table className="w-full text-left mt-4 text-gray-800 dark:text-gray-300"> <thead className="text-gray-700 dark:text-gray-200"> <tr className="bg-gray-50 dark:bg-gray-700"> <th className="p-4 font-semibold">Giảng viên</th> <th className="p-4 font-semibold">Lớp tín chỉ</th> <th className="p-4 font-semibold">Học phần</th> <th className="p-4 font-semibold">Ngày</th> <th className="p-4 font-semibold">Tiết</th> <th className="p-4 font-semibold text-center">Hành động</th> </tr> </thead> <tbody>{scheduleRows}</tbody> </table> </div> </div> );
};
